...
inline void Div(re int* F,re int* G,re int * H,re int n,re int m){
	Rev(F,n);
	Rev(G,m);
	re int N=1;
	for(;N<n;N<<=1);
	...
	Rev(G,m);
	Rev(H,n-m+1);
	for(re int i=n-m+1;i<maxn;++i)
		H[i]=0;
}

inline void Mod(re int* F,re int* G,re int* H,re int n,re int m){
	Div(F,G,H,n,m);
	re int N=1;
	for(;N<n;N<<=1);
	NTT(G,N,0);
