...
int n;
int unit[2][24],rev[maxn+1],inv[maxn+1]={0,1};
int F[maxn+1],G[maxn+1],tmp[4][maxn+1];
...
inline void FAQ(re int* F,re int n,re int tp){
...
inline void Inv(re int* F,re int* G,re int n){
	...
	for(re int i=1,j=4;i<n;i<<=1,j<<=1){
		...
		for(re int k=0;k<j;++k)
			G[k]=(2-1LL*tmp[0][k]*G[k]%mod)*G[k]%mod;
		FAQ(G,j,1);
		for(re int k=(i<<1);k<j;++k)
			G[k]=0;
	}
	for(re int i=n;G[i];G[i]=0,++i);
}
...
inline void Exp(re int* F,re int* G,re int n){
	...
	for(re int i=1,j=2;i<(n<<1);i<<=1,j<<=1){
		...
		FAQ(tmp[3],j,0);
		for(re int k=0;k<j;++k)
			G[k]=(1LL-tmp[2][k]+tmp[3][k]+mod)*G[k]%mod;
		FAQ(G,j,1);
		for(re int k=0;k<j;++k)
			cltstream::write(G[k],k<j-1?32:10);
	}
}
