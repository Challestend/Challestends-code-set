...
int main(){
    ...
    cnt[0]=cnt[a[1]]=1;
    for(re int i=1;i<=m;++i){
        for(;R<Q[i].r;++R,cnt[0]+=2*cnt[a[R]]+1,++cnt[a[R]]);
        for(;R>Q[i].r;cnt[0]-=2*cnt[a[R]]-1,--cnt[a[R]],--R);
        for(;L<Q[i].l;cnt[0]-=2*cnt[a[L]]-1,--cnt[a[L]],++L);
        for(;L>Q[i].l;--L,cnt[0]+=2*cnt[a[L]]+1,++cnt[a[L]]);
        ans[Q[i].id]=cnt[0];
    }
    for(re int i=1;i<=m;++i){
