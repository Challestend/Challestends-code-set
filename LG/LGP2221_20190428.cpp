...
class LinkCutTree{
	...
	LinkCutTree(){
		...
		for(re int i=1;i<=m;++i){
			...
			if(opt=='C'){
				cltstream::read(x);
				split(mempool+l,mempool+r);
				s=(mempool+r)->size;
				(mempool+r)->val+=x;
				(mempool+r)->sum2+=1LL*s*(s+1)*(2*s+1)/6*x;
				(mempool+r)->sum1+=1LL*s*(s+1)/2*x;
				(mempool+r)->sum0+=1LL*s*x;
				(mempool+r)->add+=x;
			}
			else{
				long long sum,cnt,g;
				split(mempool+l,mempool+r);
				s=(mempool+r)->size;
				sum=1LL*(s+1)*(mempool+r)->sum1-(mempool+r)->sum2;
				cnt=1LL*s*(s+1)/2;
				g=gcd(sum,cnt);
