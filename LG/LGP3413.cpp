...
std::string s;
int num[maxn+1],v[10][10][maxn+1],f[10][10][maxn+1];
...
int dfs(int pre2,int pre1,int pos,int touchLimit,int valid,int palin){
    if(!touchLimit&&v[pre2][pre1][pos])
        return f[pre2][pre1][pos];
    if(!pos)
        return palin;
    if(!touchLimit&&palin){
        v[pre2][pre1][pos]=1;
        f[pre2][pre1][pos]=cltpow(10,pos);
        return f[pre2][pre1][pos];
    }
    int res=0;
    if(touchLimit)
